      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header"> 
            ...
            <button class="btn btn-primary" id="createBtn" onclick="createTransaction()">
        ...
        <div class="card">
          ...
          <marquee behavior="scroll" direction="left">
            <h6 style="color:red;font-size:16px;"> This table showing the records for the User Payment Transaction Details List 2025-26</h6>
          </marquee>
          <div class="card-body">
...
function showAllTransactions(filters = {}) {
  // build query object
  let url = $('meta[name=app-url]').attr("content") + "payment-transactions/transactions_all";
  let dataParams = {};
  ...
  $.get(url, dataParams, function (data) {
    ...
    let grandTotal = 0;
    let paymentModes = {};
    paymentTotals = {}; // global used to render buttons

    // group rows by userid and build mode totals
    data.forEach(t => {
    ...
    // selectedMode is the id from dropdown; used to decide which total column to show
    let selectedMode = $("#filter_payment_mode").val();

    // Build DOM grouped by user (rowspan correct since groups come from server filtered data)
    Object.values(groups).forEach(group => {
      ...
      group.forEach((t, index) => {
        ...
        if (index === 0) {
          tr += `
            <td rowspan="${rowspan}">${t.userid}</td>
            <td rowspan="${rowspan}">
              ${escapeHtml(t.username)}<br>
              Mob: ${escapeHtml(t.mobilenumber)}
            </td>`;
        }
        ...
        tr += `
          ...
          tr += `<td rowspan="${rowspan}" class="user_total"><b>${amountToShow.toFixed(2)}</b></td>
                 <td rowspan="${rowspan}">
                   <button class="btn btn-warning btn-sm" onclick="viewClientTransactions(${t.userid})">
                     <i class="fa fa-eye"></i>
                   </button>
                   <button class="btn btn-success btn-sm" onclick="downloadReceipt(${t.userid})">
    ...
    // footer grand total (sum of visible rows because server returned filtered dataset)
    $("#paymentBody").append(`
      <tr>
        <td colspan="4" class="text-right"><b>Grand Total</b></td>
        <td class="grand_total_td"><b>${grandTotal.toFixed(2)}</b></td>
        <td><b>${grandTotal.toFixed(2)}</b></td>
        <td></td>
      </tr>
    `);

    // rebuild payment mode dropdown with ids
    ...
    // render buttons (only those present in paymentTotals for current dataset)
    renderPaymentModeButtons(paymentTotals);
...
function downloadReceipt(userid) {
    let url = $('meta[name=app-url]').attr("content") + "user-payments/receipt-download/" + userid;
...
function storeTransaction(){
  let url = $('meta[name=app-url]').attr("content") + "payment-transactions/store";
  let data = {
    userid: $("#user_id").val(),
...
function viewTransactions(userid) {
  let viewurl = $('meta[name=app-url]').attr("content") + "payment-transactions/view/" + userid;
    $.ajax({
        url: viewurl,
        type: "GET",
        dataType: "json",
        success: function(res) {
            ...
            $("#v_userid").text(res[0].userid);
            $("#v_username").text(res[0].username);
            $("#v_mobile").text(res[0].mobilenumber);
...
function loadUsers(){
  let url = $('meta[name=app-url]').attr("content") + "users/list";
  $.get(url, function(data){
    console.log(data);
    let opt = '<option value="">Select User</option>';
    data.forEach(u => opt += `<option value="${u.id}">${u.username} (${u.mobilenumber}) - [${u.id}]</option>`);
    $("#user_id").html(opt);
  }, "json");
}
